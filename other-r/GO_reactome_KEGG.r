rm(list=ls())
setwd("Z:\\16.up_down_go_reactome\\04.WT_IFNR_MDX_3-15-16-17_1-58\\MDX")
path="Z:\\16.up_down_go_reactome\\04.WT_IFNR_MDX_3-15-16-17_1-58\\MDX"
install.packages("D:\\pf\\R-4.4.1\\library\\org.Mm.eg.db_3.16.0.tar.gz", repos = NULL, type="source")
library(reactome.db)
library(ReactomePA)
library(openxlsx)
...
for (file in xlsx_files) {
  ...
  tryCatch({
    ...
    # 开始PDF设备
    pdf(file=pdf_file, width=19, height=7)
    
    # 创建并打印气泡图
    p <- ggplot(subset_data, aes(x = -log10(p.adjust), y = Description, size = Count, color = FoldEnrichment)) +
      geom_point() +
      scale_color_gradient(low = "red", high = "green") +
      labs(title = "", x = "-log10(p.adjust)", y = "") +
      theme_minimal()+
      theme(
