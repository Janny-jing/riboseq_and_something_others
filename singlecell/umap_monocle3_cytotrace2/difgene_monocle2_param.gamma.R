...
outpath <- "difgene_monocle2_results"
...
# 参数列表
param.gammas <- seq(from = 5, to = 50, by = 5)

# 循环遍历所有参数组合
for (param.gamma in param.gammas) {
    message(paste("param.gamma =", param.gamma))
    ...
    cds_subset <- orderCells(cds_subset)
    # 构建文件名
    filename <- paste0(outpath, "/param.gamma_", param.gamma, ".png")
    ...
    filename2 <- paste0(outpath, "/param.gamma_", param.gamma, "_facet.png")
