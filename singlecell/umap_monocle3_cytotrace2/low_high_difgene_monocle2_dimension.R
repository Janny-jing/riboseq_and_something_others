...
outpath <- "low_high_difgene_monocle2_results_remove31_remove27_reclusterlow_800"
if (!dir.exists(outpath)) dir.create(outpath)
print(outpath)
obj <- readRDS("subset_new_umap_dim18_spread2.5_mindist0.7_celltypes_250630_remove27_recluster_low_800.rds")
...
Idents(obj) <- obj$human_celltypes_250630
...
degs_low_vs_high <- head(arrange(low_vs_high, desc(avg_log2FC)), 500) %>%
  rownames() %>%
  as.vector()

# 取 top 400 上调基因
degs_high_vs_low <- head(arrange(high_vs_low, desc(avg_log2FC)), 500) %>%
  rownames() %>%
  as.vector()
final_gene_list <- unique(c(degs_high_vs_low,degs_low_vs_high))
print(paste("Total unique genes:", length(final_gene_list)))

diff <- differentialGeneTest(cds[final_gene_list,],
              fullModelFormulaStr = "~human_celltypes_250630")
...
# 参数列表
dimensions=seq(from = 2, to = 11, by = 1)

# 循环遍历所有参数组合
for (dimension in dimensions) {
    message(paste("Running with dimension =", dimension))
    ...
    cds_subset <- orderCells(cds_subset)
    # 构建文件名
    filename <- paste0(outpath, "/dimension_", dimension,  ".png")

    # 绘图并保存
    png(filename, width = 9 * 100, height = 7 * 100, units = "px") 
    plot1 <- plot_cell_trajectory(cds_subset, color_by = "human_celltypes_250630")
    print(plot1)
    dev.off()

    # 可选：带 facet_wrap 的版本
    plot2 <- plot_cell_trajectory(cds_subset, color_by = "human_celltypes_250630") +
      facet_wrap("~human_celltypes_250630", ncol = 3)

    filename2 <- paste0(outpath, "/dimension_", dimension,"_facet.png")
