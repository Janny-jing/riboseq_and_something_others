...
def label_transfer(query_file, figures_folder, model_type="lineage", 
    ...
    # Get latent representation of reference data with memory optimization
    print("Getting latent representations (memory optimized)...")
    data_latent_source = scpoli_query.get_latent(source_adata, mean=True)
    adata_latent_source = sc.AnnData(data_latent_source.astype(np.float32))  # Ensure float32
    ...
    adata_latent = sc.AnnData(data_latent.astype(np.float32))  # Ensure float32
    ...
    # Compute UMAP for query data if not already done
    if 'X_umap' not in adata_latent.obsm:
    ...
    # Method 1: Use is_original_query identifier (most accurate) - Get full AnnData, not just .obs
    original_query_mask = adata_latent_full.obs['is_original_query'] == True
    full_latent_correct = adata_latent_full[original_query_mask]  # Use full AnnData, not just .obs
    print(f"Filtered by is_original_query: {full_latent_correct.n_obs} cells")
    
    # Validate we got the right number of cells
    ...
    # Store UMAP coordinates (following the exact approach from the working script)
    print("Storing UMAP coordinates...")
    # Use the latent data directly (adata_latent) for UMAP coordinates, not the full concatenated data
    query_adata.obsm[f'X_umap_{cell_type_key}'] = adata_latent.obsm['X_umap']
    print(f"Added UMAP embeddings to query_adata.obsm['X_umap_{cell_type_key}']")
    print(f"UMAP shape: {adata_latent.obsm['X_umap'].shape}")
    
    # Verify UMAP was properly added
    if f'X_umap_{cell_type_key}' in query_adata.obsm:
        print(f"✅ Successfully stored UMAP coordinates for {query_adata.obsm[f'X_umap_{cell_type_key}'].shape[0]} cells")
    else:
    ...
    try:
        ...
        print(f"✅ Saved {cell_type_key} results to query data: {query_file}")
        
    except Exception as save_error:
        print(f"❌ Error saving to {query_file}: {str(save_error)}")
        
        # Try saving with compression disabled as fallback
        try:
            print("Attempting to save without compression...")
            query_adata.write_h5ad(filename=query_file, compression=None)
            print(f"✅ Saved {cell_type_key} results to query data (uncompressed): {query_file}")
        except Exception as fallback_error:
            print(f"❌ Fallback save also failed: {str(fallback_error)}")
            
            # Save essential results to CSV as last resort
            backup_csv = query_file.replace('.h5ad', '_backup_results.csv')
            try:
...
def main():
    ...
    try:
        ...
        print(f"\nResults saved to query file: {results['query_file']}")
        print("="*50)
