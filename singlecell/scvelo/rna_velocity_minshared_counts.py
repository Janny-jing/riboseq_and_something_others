...
for min_shared_counts in range(10, 101, 10):
        print(f"\nProcessing with min_shared_counts = {min_shared_counts}...")        
        ...
        scv.pp.filter_and_normalize(Sample3p_union_tmp, min_shared_counts = min_shared_counts, n_top_genes=2000)
        ...
        plot_filename = f"RNA_velocity_min_shared_counts_{min_shared_counts}.png"
        plot_save_path = os.path.join(plots_base_path, plot_filename)
        # Generate and save plot
        if 'Loupe' in Sample3p_union_tmp.obs.columns:
            print(f"Generating and saving plot with min_shared_counts = {min_shared_counts}...")
