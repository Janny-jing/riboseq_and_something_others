...
def integrate_lineage_predictions(csv_folder, original_adata_path, output_adata_path):
    """
    整合文件夹中所有CSV文件的lineage_pred列到原单细胞数据的obs中
    ...
    for i, csv_file in enumerate(csv_files):
        ...
        try:
            ...
            # 提取lineage_pred列
            if 'lineage_pred' in df.columns:
                # 使用文件名作为新列名（去除扩展名）
                col_name = f"lineage_pred_{os.path.splitext(filename)[0]}"
                
                # 重新索引以匹配原数据
                lineage_series = df['lineage_pred'].reindex(adata.obs.index)
                
                # 添加到原数据的obs中
                adata.obs[col_name] = lineage_series
                print(f"  成功添加列: {col_name}")
                
                # 统计非空值
                non_null_count = lineage_series.notna().sum()
                print(f"  有效预测值: {non_null_count}/{len(adata.obs)} 个细胞")
                
            else:
                print(f"  警告: {filename} 中未找到 lineage_pred 列")
    ...
    # 显示统计信息
    print(f"\n整合完成！")
    print(f"原单细胞数据已添加 {len([col for col in adata.obs.columns if col.startswith('lineage_pred_')])} 个lineage_pred列")
    print(f"新列名: {[col for col in adata.obs.columns if col.startswith('lineage_pred_')]}")
    
    # 显示新添加的列的前几行
    lineage_cols = [col for col in adata.obs.columns if col.startswith('lineage_pred_')]
    if lineage_cols:
        print(f"\n新添加的列数据预览:")
        print(adata.obs[lineage_cols].head())
    
    return adata

def main():
    parser = argparse.ArgumentParser(description='整合多个CSV文件中的lineage_pred列到单细胞数据中')
    ...
    print("开始整合lineage_pred数据...")
    print(f"CSV文件夹: {args.csv_folder}")
    print(f"原单细胞数据: {args.original_adata}")
    print(f"输出文件: {args.output_adata}")
    print("-" * 50)
    
    integrate_lineage_predictions(args.csv_folder, args.original_adata, args.output_adata)
